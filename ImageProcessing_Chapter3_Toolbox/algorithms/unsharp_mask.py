"""Eqs. (3-55)-(3-56) with k=1: unsharp masking."""
import numpy as np
...
def apply(image, sigma=2.0, amount=1.0):
    f=to_gray_float(image); blur=gaussian_filter(f,float(sigma),mode='reflect'); mask=f-blur
    return np.clip(f+float(amount)*mask,0,1)

if __name__ == '__main__': cli_single(apply, 'Unsharp masking', {'sigma':2.0,'amount':1.0})
