"""Eq. (3-35): 2-D spatial convolution with a user-supplied kernel."""
import numpy as np
...
def apply(image, kernel='0,-1,0;-1,5,-1;0,-1,0', boundary='reflect', display_scale=True):
    ...
    out=convolve(r,k,mode=str(boundary))
    return normalize01(out) if bool(display_scale) else np.clip(out,0,1)

if __name__ == '__main__': cli_single(apply, '2-D convolution', {'kernel':'0,-1,0;-1,5,-1;0,-1,0','boundary':'reflect','display_scale':True})
